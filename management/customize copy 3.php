...
$cssVariables = [
    ...
    'button_background-image-gradient-direction' => extract_css('--button_background-gradient-direction'),
    'button_background-image-color-stop' => extract_css('--button_background-color-stop')
        ...
        foreach ($group['settings'] as $setting) {
            ...
            switch ($setting['type']) {
                ...
                case 'gradient':
                        ...
                        // Define predefined gradient directions
                        $gradientDirections = [
                            'to top' => 'Top',
                        ...
                        foreach ($gradientDirections as $direction => $label) {
                            ...
                            echo '<div class="direction-option">';
                            echo '<input type="radio" id="' . $setting['name'] . 'gradient-direction-' . $direction . '" name="' . $setting['name'] . '-gradient-direction" value="' . $direction . '" ' . (isset($_POST[$setting['name'] . '-gradient-direction']) && $_POST[$setting['name'] . '-gradient-direction'] == $direction ? 'checked' : '') . '>';
                        ...
                        if (isset($_POST[$setting['name'] . '-color-stop'])) {
                            ...
                            foreach ($colorStops as $colorStop) {
                                echo '<input type="color" class = "color-stop" id = "'.$setting['name'].'-color-stop" name = "'.$setting['name'].'-color-stop" value ="' . $colorStop . '" onchange="handleGradient(change)">';
                            }
                        }
                        else{
                        ...
                        echo' </div>';
                        echo '<button type="button" class="add-color">Add Color</button>';
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    gradientForms.forEach(function(gradientForm) {
        ...
        addColorButton.addEventListener('click', function() {
            ...
            colorStopInput.addEventListener('input', handleGradient); // Listen for input changes
            colorStopsContainer.appendChild(colorStopInput);
        });

        // Listen for direction input change
        ...
        function handleGradient(event) {
            ...
            var colorStopHiddenInput = gradientForm.querySelector('input[name="' + settingName + '-color-stop"]');
            colorStopHiddenInput.value = JSON.stringify(colorStops);
            console.log(colorStopHiddenInput);
            // colorStopHiddenInput.value = colorStops.join(', ');
            console.log(colorStopHiddenInput);
        }
    });
});
