        ...
        foreach ($group['settings'] as $setting) {
            ...
            switch ($setting['type']) {
                ...
                case 'gradient':
                        ...
                        echo '<label for="' . $setting['id'] . '-colors">Number of Colors:</label>';
                        echo '<input type="number" id="' . $setting['id'] . '-colors" min="2" max="8" value="' . (isset($currentSettingValue['colors']) ? $currentSettingValue['colors'] : 2) . '">';  // Default to 2 colors
                      
                        // Select for direction
                        echo '<label for="' . $setting['id'] . '-direction">Direction:</label>';
                        echo '<select id="' . $setting['id'] . '-direction" name="' . $setting['name'] . '[direction]">';
                          echo '<option value="to top">Top</option>';
                          echo '<option value="to bottom" selected>Bottom</option>';
                          echo '<option value="to left">Left</option>';
                          echo '<option value="to right">Right</option>';
                          echo '<option value="to top left">Top Left</option>';
                          echo '<option value="to top right">Top Right</option>';
                          echo '<option value="to bottom left">Bottom Left</option>';
                          echo '<option value="to bottom right">Bottom Right</option>';
                        echo '</select>';
                      
                        // Loop for color pickers and hidden field (using static names)
                        echo '<input type="hidden" id="' . $setting['id'] . '" name="' . $setting['name'] . '" value="' . (isset($currentSettingValue['gradient']) ? htmlspecialchars($currentSettingValue['gradient']) : '') . '">';
                      
                        echo '<label>Color Stops:</label>';
                        for ($i = 1; $i <= (isset($currentSettingValue['colors']) ? $currentSettingValue['colors'] : 2); $i++) {
                          echo '<input type="color" id="' . $setting['id'] . '-color' . $i . '" name="' . $setting['name'] . '[colors][' . $i . ']" value="' . (isset($currentSettingValue['colors'][$i - 1]) ? $currentSettingValue['colors'][$i - 1] : '') . '">';
                        }
                        break;
                case 'text':
