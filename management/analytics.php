...
if (isset($selectedCategory) && $selectedCategory !== 'general') {
    ...
    while ($row = mysqli_fetch_assoc($resultQuestionAnswer)) {
     ...
     // Query to get data for the selected category with optional date filtering
     $queryConditionalQuestion = "SELECT cq.cqID, cq.cqContent AS question, a.answerContent AS answer,
                            COUNT(DISTINCT s.guestID) AS totalUsers,
                            COUNT(sa.saID) AS clickCount
                            FROM conditional_question cq
                            LEFT JOIN question_answer qa ON cq.cqID = qa.cqID
                            LEFT JOIN answer a ON qa.answerID = a.answerID
                            LEFT JOIN session_answers sa ON a.answerID = sa.answerID
                            LEFT JOIN session s ON sa.sessionID = s.sessionID
                            LEFT JOIN product p ON s.prodID = p.prodID
    ...
    if (!empty($timeFilterStart) && !empty($timeFilterEnd)) {
    $queryConditionalQuestion .= " AND s.timestamp BETWEEN '$formattedTimestampStart' AND '$formattedTimestampEnd'";
    }
    $queryConditionalQuestion .= " GROUP BY cq.cqID, cq.cqContent, a.answerID, a.answerContent";

    $resultConditionalQuestion = mysqli_query($conn, $queryConditionalQuestion);

    $conditionalQuestion = [];

    while ($row = mysqli_fetch_assoc($resultConditionalQuestion)) {
    $question = $row['question'];
    $answer = $row['answer'];
    $clickCount = (int)$row['clickCount'];
    $totalUsers = (int)$row['totalUsers'];
    ...
    'totalUsers' => $totalUsers,
     ...
     // Query to get data for the selected category with optional date filtering
     $queryBonusQuestion = "SELECT bq.bqID, bq.bqContent AS question, a.answerContent AS answer,
                            COUNT(DISTINCT s.guestID) AS totalUsers,
                            COUNT(sa.saID) AS clickCount
    ...
    if (!empty($timeFilterStart) && !empty($timeFilterEnd)) {
    $queryBonusQuestion .= " AND s.timestamp BETWEEN '$formattedTimestampStart' AND '$formattedTimestampEnd'";
    }
    $queryBonusQuestion .= " GROUP BY bq.bqID, bq.bqContent, a.answerID, a.answerContent";

    $resultBonusQuestion = mysqli_query($conn, $queryBonusQuestion);

    $bonusQuestion = [];

    while ($row = mysqli_fetch_assoc($resultBonusQuestion)) {
    $question = $row['question'];
    $answer = $row['answer'];
    $clickCount = (int)$row['clickCount'];
    $totalUsers = (int)$row['totalUsers'];
    ...
    // Merge all three arrays into $mergedData
    foreach ([$questionAnswerData, $conditionalQuestion, $bonusQuestion] as $data) {
        foreach ($data as $question => $answers) {
            if (!isset($mergedData[$question])) {
                // If the question doesn't exist in $mergedData, add the data
                $mergedData[$question] = $answers;
            } else {
                // If the question already exists in $mergedData, merge the answers
                $mergedData[$question] = array_merge($mergedData[$question], $answers);
            }
        }
    }
                                ...
                                <form class="form-inline d-inline">
                                    ...
                                    <select class="custom-select data mr-3" name="category" id="category" onchange="window.location.href=this.value;">
                                        ...
                                        <?php foreach ($categories as $categoryID => $categoryName) : ?>
                                            <option value="?category=<?php echo $categoryID; ?>" <?php echo ($selectedCategory == $categoryID) ? 'selected' : ''; ?>><?php echo $categoryName; ?></option>
                                        <?php endforeach; ?>
                                    </select>
                                </form>
                            </div>
        ...
        <script>
            ...
            document.addEventListener('DOMContentLoaded', function() {
                ...
                <?php if (!$hideIfGeneral) : ?> // Don't execute if no category is selected
                    var questionAnswerData = <?php echo json_encode($mergedData); ?>;
