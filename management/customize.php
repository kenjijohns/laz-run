...
$cssVariables = [
    ...
    'category-name_font-size' => extract_css('--category-name_font-size'),
    'category-name_font-weight' => extract_css('--category-name_font-weight'),
    'category-name_text-transform' => extract_css('--category-name_text-transform'),
    'category-name_color' => extract_css('--category-name_color'),
    'category-name_text-align' => extract_css('--category-name_text-align'),
    'category-name_margin' => extract_css('--category-name_margin'),
    ...
    'category-title_font-size' => extract_css('--category-title_font-size'),
    'category-title_color' => extract_css('--category-title_color'),
    'category-title_font-weight' => extract_css('--category-title_font-weight'),
    'category-title_line-height' => extract_css('--category-title_line-height'),
    'category-title_letter-spacing' => extract_css('--category-title_letter-spacing'),
    'category-title-margin' => extract_css('--category-title-margin'),
    'text-1_font-size' => extract_css('--text-1_font-size'),
    'text-1_color' => extract_css('--text-1_color'),
    'text-1_font-weight' => extract_css('--text-1_font-weight'),
    'text-1_line-height' => extract_css('--text-1_line-height'),
    'text-1_letter-spacing' => extract_css('--text-1_letter-spacing'),
    'text-1-margin' => extract_css('--text-1-margin'),
    'text-2_color' => extract_css('--text-2_color'),
    'text-2_font-size' => extract_css('--text-2_font-size'),
    'text-2_font-weight' => extract_css('--text-2_font-weight'),
    ...
    'q_container_border-color' => extract_css('--q_container_border-color'),
    'q_container_border-width' => extract_css('--q_container_border-width'),
    ...
    'next_background-color' => extract_css('--next_background-color'),
    'next_background-image-gradient-direction' => extract_css('--next_background-image-gradient-direction'),
    'next_background-image-color-stop' => extract_css('--next_background-image-color-stop'),
    ...
    'product_container_border-color' => extract_css('--product_container_border-color'),
    'product_container_border-width' => extract_css('--product_container_border-width'),
...
// Placeholder for the default values from the extracted css is $current_value[]
$current_value = [];
...
// Finding the root in the CSS which holds the CSS variables
$cssContent = ":root {\n";
    foreach ($cssVariables as $key => $pattern) {
...
// Replace the relevant parts with the updated content using regular expressions
$newCssContent = preg_replace('/:root\s*\{[^}]*\}/s', $cssContent, $currentCssContent); 

//Update styles.css with the values
// Write the updated CSS content back to styles.css
file_put_contents($cssFilePath, $newCssContent);


// id of text elements that are edited. Used to update them in the database
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    // Define an array of fields you want to update
    $fieldsToUpdate = [
    ...
    // Iterate over the fields to check if they are submitted and update them
    foreach ($fieldsToUpdate as $fieldName => $id) {
        if (isset($_POST[$fieldName])) {
            // Retrieve the textarea content from the $_POST array
            $content = nl2br($_POST[$fieldName]);
            // Update the text in the texts table
            $sql = "UPDATE `texts` SET `text` = ? WHERE `id` = ?";
        ...
        <div class="row">
            <div class="col-md-3">
                ...
                <form id="customize-form" method="post" enctype="multipart/form-data" onsubmit="submitForms()">
                        ...
                        // Update the preview iframe source based on the selected option
                            ...
                            switch ($setting['type']) {
                                ...
                                case 'text':
                                    ...
                                    break;
                                case 'textarea':
                                        // Prepare SQL statement with the setting ID as the parameter
                                        $sql = "SELECT text FROM `texts` WHERE id = ?";
                                        $stmt = $conn->prepare($sql);
                                        $stmt->bind_param("s", $setting['id']); // Assuming $setting['id'] is a string
                                        $stmt->execute();
                                        ...
                                        if ($result && $result->num_rows > 0) {
                                            ...
                                            $text = $row['text'];    // Use $text variable containing the text
                                            // Strip HTML tags from the text
                                            $text = strip_tags($text);
                                        }
                                        $stmt->close();
                                        // Output the textarea
                                        echo '<textarea id="' . $setting['id'] . '" name="' . $setting['name'] . '" class="form-control">' . htmlspecialchars($text) . '</textarea>';
                                    break;
                                case 'file':
                                ...
                                case 'gradient':
                                        ...
                                        if (isset($_POST[$setting['name'] . '-color-stop'])) {
                                            ...
                                            $hasColorStops = !empty($colorStops);
                                            if (!empty($colorStops)) {
                                        ...
                                        echo '<input type="hidden" name="' . $setting['name'] . '-color-stop" id="' . $setting['name'] . '-color-stop" value="' . htmlspecialchars(json_encode($colorStops)) . '">';
                                        echo' </div>';
                                        ...
                                        foreach ($gradientDirections as $direction => $label) {
                                            ...
                                            // Disable the direction button if there are no color stops
                                            if (!$hasColorStops) {
                                                echo 'disabled';
                                            } else {
                    ...
                    <button type="submit" name="apply" class="btn btn-primary mt-3">Save</button>
                    <script>
        ...
        document.addEventListener('DOMContentLoaded', function() {
            ...
            forms.forEach(function(form) {
                ...
                if (input.type === 'file') {
                    ...
                    // Generate a unique identifier for the textarea
                    var textareaId = form.id + '-' + textarea.name;
                    textarea.addEventListener('input', function(event) {
                        handleTextInputChange(event, textareaId);
                    });
                    textarea.addEventListener('keydown', function(event) {
                        handleTextInputKeyDown(event, textareaId);
                    });
                });
            });
        });
    ...
    gradientForms.forEach(function(gradientForm) {
        ...
        addColorButton.addEventListener('click', function() {
            ...
            removeButton.addEventListener('click', function() {
                ...
                var remainingColorStops = colorStopsContainer.querySelectorAll('.color-stop').length;
                handleGradient();
                if (remainingColorStops === 0) {
                    
                    // Disable the direction buttons
                    gradientDirectionInputs.forEach(function(input) {
                        input.disabled = true;
                        input.checked = false;
                    });
                }
            ...
            // If no direction is defined, set it to "to right"
            var selectedDirection = gradientForm.querySelector('input[name$="-gradient-direction"]:checked');
            if (!selectedDirection) {
                var toRightDirection = gradientForm.querySelector('input[name$="-gradient-direction"][value="to right"]');
                if (toRightDirection) {
                    toRightDirection.checked = true;
                    handleGradient(); // Update gradient when direction is set
                }
            }
        });
           // Attach event listener to colorStopsContainer using event delegation
           colorStopsContainer.addEventListener('input', function(event) {
            if (event.target.classList.contains('color-stop')) {
                handleGradient(); // Update gradient when color stop input value changes
            }
        });
        ...
        colorStopsContainer.addEventListener('click', function(event) {
            if (event.target.classList.contains('remove-color-stop')) {
                ...
                var remainingColorStops = colorStopsContainer.querySelectorAll('.color-stop').length;
                console.log(remainingColorStops);
                if (remainingColorStops === 0) {
                    // Disable the direction buttons
                    gradientDirectionInputs.forEach(function(input) {
                        input.disabled = true;
                        input.checked = false;
                        console.log("Gradient Directions Inputs");
                        console.log (gradientDirectionInputs);
                    });
                }
        ...
        gradientDirectionInputs.forEach(function(input) {
            input.addEventListener('change', handleGradient);
        });
